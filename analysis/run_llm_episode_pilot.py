...
EXTRACTION_SCHEMA = {
    ...
    "required": [
        ...
        "follow_path_failure_count",
        "recovery_guard_success_count",
        "wait_attempt_count",
        "wait_success_count",
        "client_deadline",
        "client_cancel",
        "planning_error_code",
        "compute_path_to_pose_active",
    ],
    "additionalProperties": False,
}
...
def extracted_episode(raw: dict[str, Any], episode_id: str) -> EpisodeRecord:
    ...
    evidence.append(
        {
            "id": "recovery-count",
            "kind": "feedback_recovery_count",
            "value": raw["maximum_feedback_recovery_count"],
            "timestamp": timestamp,
            "source": "prose_extraction",
        }
    )
    for prefix, kind, count in (
        (
            "follow-path-failure",
            "follow_path_failure",
            raw["follow_path_failure_count"],
        ),
        (
            "recovery-guard-success",
            "controller_recovery_guard_success",
        ...
        for index in range(1, count + 1):
            ...
            evidence.append(
                {
                    "id": event_id,
                    "kind": "extracted_execution_event",
                    "value": kind,
            ...
            if kind == "recovery_attempt":
                ...
                event["status"] = (
                    "completed_success"
                    if index <= raw["wait_success_count"]
                    else "started"
                )
            events.append(event)
    for flag, kind in (
...
def usage_from_events(events: list[dict[str, Any]]) -> dict[str, Any] | None:
    usages = [
        event.get("usage") for event in events if isinstance(event.get("usage"), dict)
    ]
...
def main() -> int:
    ...
    parser.add_argument(
        "--question-kind", required=True, choices=tuple(QUESTION_CONFIG)
    )
    ...
    episode_raw = json.loads(
        (args.case_dir / "structured.json").read_text(encoding="utf-8")
    )
    ...
    parity_audit = json.loads(
        (args.case_dir / "parity-audit.json").read_text(encoding="utf-8")
    )
    ...
    case = BenchmarkCase(
        f"{episode.episode_id}-{args.question_kind}",
        episode,
        prose,
        question,
        kind,
    ...
    def extract(_: str) -> EpisodeRecord:
        ...
        extraction_record = caller.call(
            "prose-extraction", extraction_prompt(prose), EXTRACTION_SCHEMA
        )
        call_refs.append(
            {
                "cache_key": extraction_record["cache_key"],
    ...
    result = {
        ...
        "parity_audit_sha256": hashlib.sha256(
            (args.case_dir / "parity-audit.json").read_bytes()
        ).hexdigest(),
        "provider": "codex-lb-via-chatgpt-login",
        "model": args.model,
