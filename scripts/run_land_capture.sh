...
docker run -d --rm --name "${capture_name}" --network host --ipc host \
    ...
    "${image}" bash -lc \
    'source /opt/ros/jazzy/setup.bash; source /workspace/astro_dock/install/setup.bash; exec /workspace/astro_dock/install/lib/crane_explain_ros/capture --output /robot-visible/capture --episode-id '"${run_id}"'-worker-0 --run-id '"${run_id}"' --bt-xml '"${bt_container}" \
