...
- A historical primary-arm packet is present at
  ...
  adjudicated analysis and predates the 12 responses from `pn-0010` and `pn-0011`. Generate a new
  complete packet/key pair together with the provider-neutral command above before annotation; do
  not invent or reconstruct the missing HMAC key.
